-- GreenCycle Database Schema for Neon (PostgreSQL)
...
CREATE TABLE devices (
    id SERIAL PRIMARY KEY,
    user_id INTEGER REFERENCES users(id) ON DELETE CASCADE,
    type VARCHAR(50) NOT NULL, -- phone, laptop, battery, etc.
    brand VARCHAR(100),
    model VARCHAR(255),
    ...
    valuation JSONB, -- {suggestion: 'donate/recycle/resell', estimated_value: number, reason: string}
    weight DECIMAL(8, 3), -- in kg
    images TEXT[], -- Array of image URLs
    qr_code TEXT UNIQUE,
    ai_detection_result JSONB, -- AI analysis results
...
CREATE TABLE impact_reports (
    ...
    user_id INTEGER REFERENCES users(id) ON DELETE CASCADE,
    device_id INTEGER REFERENCES devices(id) ON DELETE CASCADE,
    water_saved DECIMAL(10, 3), -- in liters
    co2_saved DECIMAL(10, 3), -- in kg
    toxic_saved DECIMAL(10, 3), -- in kg
    energy_saved DECIMAL(10, 3), -- in kWh
    raw_materials_saved JSONB, -- {metal: kg, plastic: kg, glass: kg}
...
CREATE TABLE communities (
    ...
    name VARCHAR(255) NOT NULL,
    description TEXT,
    type VARCHAR(50), -- neighborhood, corporate, school, etc.
...
CREATE TABLE challenges (
    ...
    description TEXT,
    type VARCHAR(50), -- individual, community, global
    target_value INTEGER,
    target_metric VARCHAR(50), -- devices, weight, points
...
-- Eco Impact Formulas table
CREATE TABLE eco_impact_formulas (
    id SERIAL PRIMARY KEY,
    device_type VARCHAR(50) NOT NULL,
    weight_category VARCHAR(20), -- light, medium, heavy
    ...
    source VARCHAR(255), -- research source
...
CREATE TABLE analytics_events (
    ...
    entity_type VARCHAR(50), -- device, pickup, ngo, etc.
    entity_id INTEGER,
    properties JSONB,
...
CREATE TABLE notifications (
    ...
    type VARCHAR(50), -- pickup_reminder, status_update, achievement, etc.
...
-- Insert sample eco-impact formulas
